...
if response.status_code ==200:
    ...
    for user in users:
        # if user['name'] =="Leanne Graham":
            print(f"Nome: {user['name']}, E-mail: {user['email']}, Telefone: {user['phone']}")        
else:
...
if response.status_code == 200:
    ...
    # Adicionando os cabeçalhos
    sheet.append(["Concurso", "Valor do Prêmio", "Estados Premiados"])

    # Adicionando dados da API ao Excel
    for loteria in loterias:
        sheet.append(f"{loteria['concurso']}, {loteria['valorPremio']}, {loteria['estadosPremiados']}")
